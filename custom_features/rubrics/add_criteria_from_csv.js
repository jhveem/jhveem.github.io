(function () {
  ...
  var currentRubricId = '';
  var currentCourseId = window.location.pathname.match(/courses\/([0-9]+)/)[1];
  ...
  var assocRegex = new RegExp('/(rubrics|assignments\/)');
  var errors = [];
  var outcomes = [];
  ...
  function addRubric(criteria, association) {
  ...
  function addRatings(descriptions, points) {
    ...
    for (var i = 0; i < n; i++) {
      j = mono < 0 ? i : n - 1 - i;
      let _description = descriptions[j];
      let _descriptionPieces = _description.split(";");
      if (_descriptionPieces.length == 1) {
        _descriptionPieces[1] = '';
      }
      ratings.push({
        'description': _descriptionPieces[0].replace(/\\n/g, ' ').replace(/\s+/g, ' '),
        'long_description': _descriptionPieces[1].replace(/\\n/g, ' ').replace(/\s+/g, ' '),
        'points': points[j],
      });
    }
    return ratings;
  ...
  async function add_button() {
    let container = await getElement(".rubric_container");
    currentRubricId = container.attr("id").replace("rubric_", "");
    let rubricData = await $.get("/api/v1/courses/" + currentCourseId + "/rubrics/" + currentRubricId, function (data) {
  ...
  function processDialog() {
    ...
    errors = [];
    var txt, assocMatch, association;
    var el = document.getElementById('jj_rubric_text');
  ...
  function prepareRubric(criteria, association) {
    if (typeof criteria === 'object' && criteria.length > 0) {
      var formData = addRubric(existingCriteria.concat(criteria), association);
    ...
    try {
      ...
      while (isValid && i < lines.length) {
        ...
        if (words.length > k) {
          ...
          if (outcome && words.length > k + 1 && !isPoints(words[k]) && isPoints(words[k + 1])) {} else {
            name = words[k++];
          }
        }
  ...
  function checkPendingOutcomes() {
    ...
    if (pendingOutcomes <= 0) {
      ...
      if (outstanding.length > 0) {
        ...
        prepareRubric(criteria, rubricAssociation);
      }
    }
  }
