      ...
      <table class='sortable' border='1' id='btech-report-table'>
        ...
        <tbody border='1'>
          <tr v-for='(student, id) in students'>
            <th v-for='column in columns' :key='column.name' :style='{"background-color": getDaysSinceLastSubmissionColor(column, student[column.name.toLowerCase().replace(/ /g, "_")]})'>{{student[column.name.toLowerCase().replace(/ /g, "_")]}}</th>
          </tr>
        </tbody>
        <tfoot border='1'>
